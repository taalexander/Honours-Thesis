...
\begin{equation}
d_1 = \frac{D}{cos(-\theta -\delta)} \,\,\,\,\, d_2 = \frac{D}{cos(-\theta +\delta)}
\label{eq:phase_path_lengths}
\end{equation}
Where $D$ is the thickness of the phase flag, $\theta$ is the angle of the two neutron beams relative to the horizontal, and $\delta$ is the angle of the phase flag relative to the vertical. Using these paths lengths and (\ref{eq:phase_shift_full}) the phase shift along each path are 
\begin{equation*}
\delta\phi_1 = \frac{Nb\lambda D}{ cos(-\theta -\delta)} \,\,\,\,\, \delta\phi_2 = \frac{Nb\lambda D}{ cos(-\theta +\delta)}
\end{equation*}
Which gives a relative phase between the two beams of 
\begin{equation}
\delta\phi = Nb\lambda D \left(\frac{1}{cos(-\theta -\delta)} - \frac{1}{cos(-\theta +\delta)}\right)  
