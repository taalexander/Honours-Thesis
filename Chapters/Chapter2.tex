...
\begin{equation}
I_O = \Braket{\Psi_O | \Psi_O} = |\Psi_0|^2|r|^4|t|^2(1+cos(\psi_1-\psi_2) = A(1+cos(\Delta\phi)
\label{eq:obeamintensity}
\end{equation} 
Where $A=|\Psi_0|^2|r|^4|t|^2$ and $\Delta\phi = \phi_1-\phi_2$. The resultant intensity for the $H$ beam is therefore 
\begin{equation}
I_H = \Braket{\Psi_H | \Psi_H} = |\Psi_0|^2(|t|^4|r|^2 + |r|^6)-|\Psi_0|^2|r|^4|t|^2(cos(\phi_1-\phi_2)) = B - Acos(\Delta\phi)
\label{eq:hbeamintensity}
\end{equation}
Where $B=|\Psi_0|^2(|t|^4|r|^2 + |r|^6)$. 
...
\begin{equation}
\delta\phi = Nb\lambda D \left(\frac{1}{cos(\theta_B -\delta)} - \frac{1}{cos(\theta_B +\delta)}\right) = -2Nb\lambda \frac{sin(\theta_B)sin(\delta)}{cos^2(\theta_B)-sin^2(\delta)} \approx (CONSTANT)\times \delta 
\label{eq:relative_phase}
\end{equation}
The contrast of the neutron interferometer is defined as a function of the phase flag angle and measured intensities at the neutron detectors\cite{noise_neutron}
\begin{equation}
C_0 = \frac{\text{max}[D_0(\delta)] - \text{min}[D_0(\delta)]}{\text{max}[D_0(\delta)] + \text{min}[D_0(\delta)]}
\label{eq:contrast}
\end{equation}
Where $D_0$ is the detected neutron intensity at the $O$ detector. 
